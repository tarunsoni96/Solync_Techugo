import React, { Component } from 'react';
import {
  StyleSheet,
  Text,
  View,
  SafeAreaView,
  Image,
  Dimensions,
  ScrollView,
  ImageBackground,
  TouchableOpacity,
  AsyncStorage,
  FlatList,
  BackHandler,
  Modal
} from 'react-native';
import { widthPercentageToDP as wp, heightPercentageToDP as hp } from 'react-native-responsive-screen'
import Carousel,{ ParallaxImage} from 'react-native-snap-carousel';
import Dash from 'react-native-dash';
import LinearGradient from 'react-native-linear-gradient';
import { withNavigation, NavigationEvents } from 'react-navigation';
import GestureRecognizer, {swipeDirections} from 'react-native-swipe-gestures';

const { height, width } = Dimensions.get('screen')
class MUSIC extends Component {
  constructor(props) {
    super(props);
    this.state = {
      myText: 'I\'m ready to get swiped!',
      gestureName: 'none',
      backgroundColor: '#fff',
      videos: [
        {
          id: "WpIAc9by5iU",
          thumbnail: "data:image/jpeg;base64,",
          title: "Sally"
        }, {
          id: "sNPnbI1arSE",
          thumbnail: "data:image/jpeg;base64,",
          title: "Sally"
        }, {
          id: "VOgFZfRVaww",
          thumbnail: "data:image/jpeg;base64,",
          title: "Sally"
        }
      ],
      showViewTop: true,
      userId: '',
      noMatches: false,
      showModal:false


    }
    this.props = props;
    this._carousel = {};

  }
  componentDidMount() {
    BackHandler.addEventListener('hardwareBackPress', this.handleBackButton)
    AsyncStorage.getItem('userId', (err, result) => {
      userData = JSON.parse(result)
      console.log('USER ID CONSOLE', userData)
      this.setState({
        userId: userData
      })
      this._fetchData()
    });
  }

  handleBackButton() {
    return true;
  }
  componentWillUnmount() {
    BackHandler.removeEventListener('hardwareBackPress', this.handleBackButton)
  }

  _fetchData() {


    console.log(JSON.stringify(this.state.userId))
    fetch('http://13.232.62.239:6565/api/user/home', {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        "user_id": this.state.userId,
        "filterCategory": 1


      }),

    }).then((response) => response.json())

      .then((responseJson) => {
        let arrData = []
        
        console.log('HOME RESPONSE', JSON.stringify(responseJson))
        if (responseJson.statusCode == 200) {
          console.log('SUCCESS RESPONSE', JSON.stringify(responseJson))
          var category = responseJson.result.userInfo.categoryName
          var CatArr = this.catStrToArray(category)
          var data = CatArr;

          var found = data.find(function (element) {
            if (element == 'Music') {
              console.log('Found')
              return
            }
            else {
              console.log('Not Found')
              return
            }
          });

          console.log(found);
          for (i = 0; i <= responseJson.result.home.length - 1; i++) {
            var data1 = responseJson.result.home[i]
            arrData.push(data1)
          }


          console.log('ARR DATA===', JSON.stringify(arrData))
          
          this.setState({
            videos: arrData,
            showModal:true

          })
          
          console.log('EVENT LIST ===', JSON.stringify(this.state.videos))
        }
        else if (responseJson.statusCode == 201) {
          console.log('FALIURE RESPONSE', JSON.stringify(responseJson))
          this.setState({
            noMatches: true
          })
          
        }
        else {
          console.log('@@@@@@@@@', JSON.stringify(responseJson))
         
        }
        return responseJson;

      })
      .catch((error) => {
        console.log(error);
      });


  }

  onSwipeUp(gestureState) {
    alert("Hi");
    this.setState({ myText: 'You swiped up!' });
    this.props.navigation.navigate('MusicGenre')
  }
  onSwipeDown(gestureState) {
    alert("Hi");
    alert('ddf')
  }


  onSwipe(gestureName, gestureState) {
    alert("Hi");
    const { SWIPE_UP, SWIPE_DOWN, SWIPE_LEFT, SWIPE_RIGHT } = swipeDirections;
    this.setState({ gestureName: gestureName });
    switch (gestureName) {
      case SWIPE_UP:
        this.setState({ backgroundColor: '#fff' });
        break;
      case SWIPE_DOWN:
        this.setState({ backgroundColor: '#fff' });
        break;

    }
  }

  handleSnapToItem(index) {
    console.log("snapped to ", index)
  }
  _closeView() {
    this.setState({
      showViewTop: false
    })
    console.log('_closeView', this.state.showViewTop)
  }
  _formatMonth(date) {

    var monthNames = [
      "Jan", "Feb", "Mar",
      "Apr", "May", "Jun", "Jul",
      "Aug", "Sept", "Oct",
      "Nov", "Dec"
    ];
    date = new Date(date)
    var day = date.getDate();
    var monthIndex = date.getMonth();
    var year = date.getFullYear();
    console.log('======', JSON.stringify(day + ' ' + monthNames[monthIndex] + ' ' + year))

    return monthNames[monthIndex];
  }
  _formatYear(date) {
    var monthNames = [
      "January", "February", "March",
      "April", "May", "June", "July",
      "August", "September", "October",
      "November", "December"
    ];
    date = new Date(date)
    var day = date.getDate();
    var monthIndex = date.getMonth();
    var year = date.getFullYear();
    console.log('======', JSON.stringify(day + ' ' + monthNames[monthIndex] + ' ' + year))

    return year;
  }
  catStrToArray(cat) {
    if (cat == undefined) {
      return
    }
    else {
      var arraySub = cat.split(",");

      return arraySub;
    }
  }
  _strToArr(subCat) {
    let newArr = []
    if (subCat == undefined) {
      return
    }
    else {
      var arraySub = subCat.split(",");
      for (i = 0; i < arraySub.length; i++) {
        newArr.push(arraySub[i])
        if (i == 1)
          break;

      }
      return newArr;
    }
  }
  _renderItemSub(item, index) {
    console.log('=====', item)
    return (
      <View style={{ width: '40%', height: 25, backgroundColor: 'transparent', alignSelf: 'flex-start', justifyContent: 'center' }}>
        <View style={{ height: '100%', width: '96%', backgroundColor: 'transparent', borderColor: '#c8aecb', borderWidth: 2, borderRadius: 4, justifyContent: 'center' }}>
          <Text style={{ color: '#781773', fontSize: 15, fontFamily: 'Montserrat-Bold', alignSelf: 'center' }}>{item.item}</Text>
        </View>
      </View>
    )
  }
//   _renderItem = ({ item, index }) => {

//     var str = item.date;
//     var formatYear = this._formatYear(str)
//     var formatMonth = this._formatMonth(str)
//     var subCat = item.subCategory
//     var subCatArr = this._strToArr(subCat)

//     console.log("rendering,", subCatArr)
//     return (
//       <ScrollView stle={{ flex: 1 }} showsVerticalScrollIndicator={false}>
//         <View style={styles.ThumbnailBackgroundView}>
//           <TouchableOpacity
//             onPress={() => {
//               console.log("clicked to index", index)
//               this._carousel.snapToItem(index);
//             }}
//           >
//             {item.profile_picture == '' ?
//               <ImageBackground style={styles.CurrentVideoImage} source={require('../../assets/Images/@photo-cropped.png')}
//                 resizeMode={'contain'}>
//                 <Image source={require('../../assets/Images/@BG-gradient.png')} style={{ height: width - 55, borderRadius: 20, width: width - 50 }} />
//               </ImageBackground> :
//               <ImageBackground style={styles.CurrentVideoImage} source={{ uri: item.profile_picture }}
//                 resizeMode={'contain'}>
                  
//                 {/* <Image source={require('../../assets/Images/@BG-gradient.png')} style={{ height: width - 50, borderRadius: 20, width: width - 50,top:hp(1) }} /> */}
              
//               </ImageBackground>
//             }
          

//             <View style={{ width: width - 85, height: '20%', position: 'absolute', bottom: 0, backgroundColor: 'transparent', flexDirection: 'row', justifyContent: 'flex-start', alignSelf: 'center' }}>
//               <Text style={{ color: '#fff', fontSize: 22, fontFamily: 'Montserrat-ExtraBold', left: '5%', backgroundColor: 'transparent' }}>{item.first_name}, {item.age}</Text>
//               <View style={{ height: '100%', width: width / 2, backgroundColor: 'transparent', alignSelf: 'center' }}>
//                 <Text style={{ color: 'grey', fontSize: 16, alignSelf: 'flex-start', backgroundColor: 'transparent', top: '5%' }}>    {item.occupation}</Text>
//               </View>
//             </View>
//           </TouchableOpacity>

//           {/* <View style={{ height: '40%', width: '95%', backgroundColor: 'transparent', alignSelf: 'center', bottom: hp('1.7%') }}> */}



//             {/* <ImageBackground
//               source={require('../../assets/Images/@Group.png')}
//               resizeMode='contain'
//               style={{ height: hp('28%'), padding: 0, width: '100%', alignSelf: 'center', justifyContent: 'center', alignItems: 'center' }}

//             >
//               <View style={{ flexDirection: 'row', height: 80, width: '75%', alignSelf: 'center', backgroundColor: 'transparent', borderRadius: 10, justifyContent: 'center' }}>
//                 <View style={{ height: '80%', width: '90%', justifyContent: 'flex-end', alignSelf: 'center', borderRadius: 20, backgroundColor: 'transparent' }}>

//                   <Text style={{ color: '#d39dc5', fontSize: 14, fontWeight: 'bold', alignSelf: 'flex-start', width: '70%' }}>Event:</Text>
//                   <Text style={{ color: '#fff', fontSize: 13, fontWeight: 'bold' }}>{item.artist_or_event}</Text>
//                 </View>
//               </View>
//               <View style={{ height: 140, width: '80%', justifyContent: 'space-evenly', alignSelf: 'center', backgroundColor: 'transparent', borderRadius: 10 }}>
//                 <View style={{ height: '100%', width: '100%', justifyContent: 'space-around', alignSelf: 'center', backgroundColor: 'transparent' }}>
//                   <View style={{ justifyContent: 'space-between', flexDirection: 'row', width: '85%', alignSelf: 'flex-start', height: '20%' }}>
//                     <View style={{ width: width / 3.8, backgroundColor: 'transparent', alignSelf: 'center' }}>
//                       <Text style={{ color: '#8c979e', fontSize: 13, fontFamily: 'Montserrat-Bold', alignSelf: 'flex-start' }}>Month</Text>
//                       <Text style={{ color: '#343434', fontSize: 12, fontFamily: 'Montserrat-Bold', alignSelf: 'flex-start' }}>{formatMonth}</Text>
//                     </View>
               
//                     <View style={{ width: width / 5, backgroundColor: 'transparent', alignSelf: 'center' }}>
//                       <Text style={{ color: '#8c979e', fontSize: 13, fontFamily: 'Montserrat-Bold', alignSelf: 'flex-start' }}>Year</Text>
//                       <Text style={{ color: '#343434', fontSize: 12, fontFamily: 'Montserrat-Bold', alignSelf: 'flex-start' }}>{formatYear}</Text>
//                     </View>
//                     <View style={{ width: width / 3.5, alignSelf: 'center' }}>
//                       <Text style={{ color: '#8c979e', fontSize: 13, fontFamily: 'Montserrat-Bold', alignSelf: 'flex-start' }}>Location</Text>
//                       <Text style={{ color: '#343434', fontSize: 12, fontFamily: 'Montserrat-Bold', alignSelf: 'flex-start' }}>{item.location}</Text>
//                     </View>
//                   </View>
//                   <View style={{ width: '100%', bottom: 10 }}>

//                     <FlatList
//                       data={subCatArr}
//                       renderItem={(item, index) => this._renderItemSub(item, index)}
//                       extraData={this.state}
                     
//                       numColumns={2}
                   

//                     />

          
//                   </View>

//                 </View>




//               </View>
//             </ImageBackground> */}

// {/* //////////////////////////////////------code of chat button---------------- */}
//             {/* <View style={{ height: height / 6, width: width - 20, backgroundColor: 'transparent', justifyContent: 'center', alignSelf: 'center' }}>
//               <TouchableOpacity>
//                 <ImageBackground source={require('../../assets/Images/GradientButton.png')} style={{ height: height / 8.5, width: '100%', alignSelf: 'center', borderRadius: 48, justifyContent: 'center' }}>
//                   <Text style={{ textAlign: 'center', opacity: 1, fontSize: 19, fontFamily: 'Montserrat-Bold', color: '#fff', alignSelf: 'center' }}>Chat</Text>
//                 </ImageBackground>
//               </TouchableOpacity>
//             </View> */}
// {/* --------------------------------------------------------------------------- */}
//           {/* </View> */}




//         </View>
//       </ScrollView>


//     );
//   }


_renderItem ({item, index}, parallaxProps) {
    return (
        <View style={styles.item}>
            <TouchableOpacity style={{flex:1}} activeOpacity={0.6}>
            <ParallaxImage
                source={require('../../assets/Images/@BG-gradient.png')}
                containerStyle={styles.imageContainer}
                style={styles.image}
                parallaxFactor={0.4}
                {...parallaxProps}
            />
            <Image source={require('../../assets/Images/@BG-gradient.png')} style={{ height: width - 50, borderRadius: 20, width: width - 50,position:'absolute' }} />
          <Text style={{bottom:'18%',left:'8%'}}><Text style={{color: '#fff',fontSize: 24, fontFamily: 'Montserrat-ExtraBold', left: '5%', backgroundColor: 'transparent'}} numberOfLines={2}>
            {item.first_name}, {item.age}</Text><Text style={{ color: 'grey', fontSize: 16, alignSelf: 'flex-start', backgroundColor: 'transparent', top: '5%' }}>    {item.occupation}</Text>               
            </Text>
            {/* <Image source={require('../../assets/Images/@BG-gradient.png')} style={{overflow:'hidden', height: width - 50, borderRadius: 20, width: width - 50,position:'absolute' }} /> */}
            </TouchableOpacity>
        </View>
    );
}
  setModalVisible(visible) {
    this.setState({showModal: visible});
  }
  render() {
    const config = {
      velocityThreshold: 0.3,
      directionalOffsetThreshold: 80
    };
    console.log("videos: updating")
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }}>
        <NavigationEvents
          onWillFocus={payload => this._fetchData()}
         //onDidFocus={payload => alert('did focus', payload)}
         //onWillBlur={payload => alert('will blur', payload)}
         //onDidBlur={payload => alert('did blur', payload)}
        />
        {this.state.noMatches == false ?
        <View>
          {/* <ScrollView style={{ backgroundColor: 'transparent' }} showsVerticalScrollIndicator={false}> */}


            {/* <View style={styles.CarouselBackgroundView}> */}


{/* --------------------------------------------- */}
             <Carousel
                sliderWidth={width}
                sliderHeight={width-200}
                itemWidth={width - 60}
                data={this.state.videos}
                renderItem={this._renderItem}
                hasParallaxImages={true}
            />

{/* ----------------------------------------------------- */}
              {/* <Carousel
                ref={(c) => { this._carousel = c; }}
                data={this.state.videos}
                renderItem={this._renderItem.bind(this)}
                sliderWidth={width - 0}
                itemWidth={width - 50}

                layout={'default'}


              /> */}


            {/* </View> */}

          {/* </ScrollView>  */}
          <View style={{ height: height / 6, width: width - 20, backgroundColor: 'transparent', justifyContent: 'center', alignSelf: 'center',bottom:hp('3%') }}>
        <TouchableOpacity>
          <ImageBackground source={require('../../assets/Images/GradientButton.png')} style={{ height: height / 8.5, width: '100%', alignSelf: 'center', borderRadius: 48, justifyContent: 'center' }}>
            <Text style={{ textAlign: 'center', opacity: 1, fontSize: 19, fontFamily: 'Montserrat-Bold', color: '#fff', alignSelf: 'center' }}>Chat</Text>
          </ImageBackground>
        </TouchableOpacity>
      </View>
      </View>
          :



          <View style={{ alignSelf: 'center', justifyContent: 'center' }}>
            <View style={{ alignContent: 'center', alignItems: 'center', paddingTop: height * 0.1, justifyContent: 'center' }}>


              <Image source={require('../../assets/Images/@nomatches.png')} style={{ height: 90, width: 90 }} />

              <Text style={{ fontSize: 25, paddingTop: 40, paddingBottom: 20, fontFamily: 'Montserrat-ExtraBold' }}>No matches</Text>
              <Text style={{ color: '#7e7e7e', fontSize: 15, fontFamily: 'Montserrat-Regular', textAlign: 'center' }}>Solync is unable to find any matches.{'\n'}
                Please change your search and filter{'\n'}
                preferences or try again later</Text>
              <Text style={{ alignSelf: 'center', paddingBottom: 20, color: '#7e7e7e', fontSize: 15, fontFamily: 'Montserrat-Regular' }}></Text>


              <TouchableOpacity onPress={() => this.props.navigation.navigate('Settings')} style={{ height: height / 8, width: width - 30, justifyContent: 'center' }} >
                <ImageBackground source={require('../../assets/Images/GradientButton.png')} style={{ height: height / 8, width: '100%', alignSelf: 'center', borderRadius: 48, justifyContent: 'center' }}>
                  <Text style={{ textAlign: 'center', opacity: 1, fontSize: 17, fontFamily: 'Montserrat-Bold', color: '#fff', alignSelf: 'center' }}>Change filter settings</Text>
                </ImageBackground>
              </TouchableOpacity>




              <TouchableOpacity onPress={() => this.props.navigation.navigate('MultipleView', { type: 'Music', typeId: '1' })} style={{ height: height / 8, width: width - 30, alignSelf: 'center', justifyContent: 'center' }}>
                <ImageBackground source={require('../../assets/Images/GradientButton.png')} style={{ height: height / 8, width: '100%', alignSelf: 'center', borderRadius: 48, justifyContent: 'center' }}>
                  <Text style={{ textAlign: 'center', opacity: 1, fontSize: 17, fontFamily: 'Montserrat-Bold', color: '#fff', alignSelf: 'center' }}>Change music preferences</Text>
                </ImageBackground>
              </TouchableOpacity>


            </View>
          </View>


        }

<Modal
animationType='slide'
transparent={true}
visible={this.state.showModal}
onRequestClose={()=>{
  alert('Modal has been closed')
}}>
  {/* <View style={{marginTop: 22}}>
            <View>
              <Text>Hello World!</Text>

              <TouchableOpacity
                onPress={() => {
                  this.setModalVisible(!this.state.showModal);
                }}>
                <Text>Hide Modal</Text>
              </TouchableOpacity>
            </View>
          </View> */}
          <View style={{height:height,width:width,backgroundColor:'#5a5b5b',opacity:0.60,position:'relative',justifyContent:'center'}}>

</View>
<TouchableOpacity  onPress={() => {
                  this.setModalVisible(!this.state.showModal);
                }} 
                style={{position:'absolute',top:'5%',right:'5%',height:40,width:40}}>
<Image source={require('../../assets/Images/modalClose.png')} style={{height:40,width:40,position:'absolute',top:'5%',right:'5%'}} resizeMode={'contain'}/> 
</TouchableOpacity>

<Image source={require('../../assets/Images/swipeDown.png')} style={{height:'15%',width:'17%',position:'absolute',top:'15%',left:'10%'}} resizeMode={'contain'}/>
<Dash style={{width:width-75, height:10, flexDirection:'row',position:'absolute',alignSelf:'center',top:'35%'}}
dashColor={'lightgrey'}
dashThickness={5}
dashGap={5}
dashLength={15}/>

<Image source={require('../../assets/Images/Tap.png')} style={{height:'15%',width:'17.5%',position:'absolute',top:'40%',left:'10%'}} resizeMode={'contain'}/>
</Modal>
      </SafeAreaView>
    );
  }
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
  },
  card: {
    alignItems: 'center',
    justifyContent: 'center',
    alignSelf: 'center',
    flex: 1,
    position: 'relative'
  },
  text: {
    textAlign: 'center',
    margin: 10,
    height: 75
  },
  instructions: {
    textAlign: 'center',
    color: '#333333',
    marginBottom: 5
  },
  VideoTitleText: {
    color: "#fff",
    fontWeight: 'bold',
    fontSize: 25,

    justifyContent: 'flex-start',
    position: 'absolute',
    //bottom:35,
    left: 20

  },
  VideoTitleProf: {
    color: "#000",

    fontSize: 15,
    right: 0,
    justifyContent: 'flex-end',
    position: 'absolute',
    //bottom:30,


  },
  CurrentVideoImage: {

    width: width - 50,
    height: width - 55,
    borderRadius: 20,
    position: 'relative',
    alignSelf: 'center',
    justifyContent: 'center',
    


  },
  ThumbnailBackgroundView: {

    alignItems: 'center',
    width: width - 40,
    height: height,
    alignSelf: 'center',
    flex: 1,
    backgroundColor: 'transparent'
  },
  CarouselBackgroundView: {

    backgroundColor: 'pink',
    height: height - 90,
    width: '100%',
    justifyContent: 'center',
    alignSelf: 'center',
    position: 'relative',
    backgroundColor: 'transparent'
  },
  item: {
    width: width - 60,
    height: width - 60,
    borderRadius:10,
    overflow:'hidden'
  },
  imageContainer: {
    flex:1,
    marginBottom: Platform.select({ ios: 0, android: 1 }), // Prevent a random Android rendering issue
    backgroundColor: 'white',
    borderRadius: 8,
    
  },
  image: {
    ...StyleSheet.absoluteFillObject,
    resizeMode:'contain',
  },
});

export default withNavigation(MUSIC);
